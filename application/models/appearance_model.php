    ...
    public function formValidation($playerCounts)
    {
        ...
        foreach ($playerCounts as $appearanceType => $playerCount) {
            ...
            while($i < $playerCount) {
                ...
                $this->ci->form_validation->set_rules("order[{$appearanceType}][{$i}]", "Order", 'tri|integer|xss_clean');
                $this->ci->form_validation->set_rules("shirt[{$appearanceType}][{$i}]", "Shirt", 'trim|integer|xss_clean');

                $this->ci->form_validation->set_rules("on[{$appearanceType}][{$i}]", "Substituted On", 'trim|integer|xss_clean');
                $this->ci->form_validation->set_rules("off[{$appearanceType}][{$i}]", "Substituted Off", 'trim|integer|xss_clean');
